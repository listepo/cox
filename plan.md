...
- A18 2026-09-11 §2, `AGENTS.md` — the human is the only author: no agent adds a `Co-Authored-By` trailer, a "Generated with …" line or itself as author to a commit, merge or PR, whatever its harness defaults to. Why: user request. Effect: a rule that never bends in `AGENTS.md`; existing history is left as it is.
