...
| T22.4 | todo | P1 | 2 | 0% | |
| T22.7 | todo | P1 | 1 | 0% | |
| T23.0 | in progress | P1 | 2 | 0% | Claude Code / claude-sonnet-5 |
| T23.1 | todo | P1 | 2 | 0% | |
| T23.2 | todo | P1 | 2 | 0% | |
| T23.3 | todo | P1 | 2 | 0% | |
| T23.4 | todo | P2 | 1 | 0% | |
...
Model: sonnet · Status: in progress · Depends: — · Size: ~150 · Priority: P1 · Complexity: 2
...
Execution plan:
- `crates/cox-tui/src/term.rs` (new, `pub mod term;` in `lib.rs`): `Caps` (10 `bool` fields, `Copy`). `detect(env)` — pure env heuristic grouping terminals into a `kitty_family` (Kitty/Ghostty/WezTerm/foot: keyboard protocol + graphics) and a broader `modern` set (+ iTerm2/VS Code/Alacritty/Windows Terminal: truecolor + OSC 8/52/9) that a small table test exercises for Ghostty, Kitty, WezTerm, iTerm2, Terminal.app, Alacritty, Windows Terminal, VS Code, foot, tmux-inside-Ghostty, SSH and `NO_COLOR`. `query(&mut self, timeout)` reuses `crossterm::terminal::supports_keyboard_enhancement()` (already does the `CSI ?u` + primary-DA dance) off a helper thread joined with `recv_timeout(timeout)` so a non-responding terminal never blocks past our own budget (crossterm's own timeout is a fixed 2 s); skipped outright when stdout is not a tty. `apply(&mut self, overrides: &HashMap<String, bool>)` sets named fields from `[tui.caps]`, ignoring unknown keys (fail open). `fields(&self)` for iteration/display.
- `crates/cox-protocol/src/config.rs` + `default.toml`: `TuiConfig.caps: HashMap<String, bool>` (mirrors `icons`), default empty, documented inline like `icons`.
- `crates/cox/src/doctor.rs`: `check_terminal` builds `Caps::detect` → bounded `query` (tty only) → `apply(config overrides)`, then prints one `name=value (source)` row per field, source being the first of `config`/`query`/`env` that actually set it.
- `crates/cox/src/main.rs`: thread `&loaded.config.tui.caps` into `doctor::run`/`check_terminal` (one-line call-site change, same pattern T22.6 used for `tui_theme`).
- `docs/config.md`: regenerated by `config_docs_config_md_matches_default_toml` after the `default.toml` edit (delete + rerun the test, do not hand-edit).
- Verify: the Check below, `cargo nextest run --workspace`, `cargo clippy --workspace --all-targets -- -D warnings`, `cargo fmt --check`, `COX_HOME` scratch `doctor` run.
