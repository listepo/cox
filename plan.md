...
Model: sonnet · Status: open · Depends: T0.2 · Size: ~200 (+ generated `schema.rs`, migrations)
Goal: §1.7 schema opens, migrates and round-trips rows through Diesel models; rollouts append and read back.
Files: `crates/cox-store/src/{lib,models,rollout}.rs`, `crates/cox-store/src/schema.rs` (generated), `crates/cox-store/migrations/00000000000001_init/{up,down}.sql`.
Steps: (1) Deps per §1.1 (replace `rusqlite` in the workspace manifest with `diesel`, `diesel_migrations`, `libsqlite3-sys` bundled); `Store::open(home)`: create dirs, `SqliteConnection::establish`, `PRAGMA journal_mode=WAL; foreign_keys=ON; busy_timeout=5000`, `run_pending_migrations` from `embed_migrations!()`. (2) Models for `sessions`, `usage`, `archive`, `memory` (`Queryable`, `Insertable`, `AsChangeset` where needed); `session_create/update`, `usage_insert`, `archive_put/get` (inline ≤ 16 KiB, else `archive/<id>` file, sha256 verified on read), `memory_*` stubs; FTS5 tables (`memory_fts`, `rollout_fts`) created in `up.sql` and used through `sql_query`. (3) `rollout_append` writes one JSON line with `seq`, fsyncs every 16 lines and on `TurnDone`; `rollout_read` tolerates a truncated last line. (4) Tests in a tempdir: `migrations_are_idempotent`, `schema_rs_matches_migrations` (compare `diesel print-schema` output or the `sqlite_master` SQL snapshot), `archive_roundtrip_inline_and_file`, `rollout_survives_truncated_tail`.
Check:
```bash
mise exec -- cargo test -p cox-store && COX_HOME=$(mktemp -d) mise exec -- cargo run -q -- doctor | grep 'db: ok'
```
Done when: `sqlite_master` SQL snapshot matches §1.7; no `diesel` dependency outside `cox-store` (T0.1 direction test extended).
