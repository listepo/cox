...
| `crates/cox-plugin` | the WASM plugin host (A52, T33.3): `PluginHost` loads a module from bytes with extism (default features off: no ureq, no URL or file loading) and gives each plugin one worker thread that owns its `Plugin` and serves a control queue before an event queue, with a per-call deadline through `CancelHandle`, a memory cap and `PluginError` mapped from `extism::Error`. The only crate that depends on extism and wasmtime (`deps.rs`); may depend on `cox-protocol`, `cox-plugin-api` and `cox-sanitize` only |
