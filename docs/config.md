...
- `mouse` = `false` — true: the wheel scrolls the transcript, the diff view and pickers 3 lines/rows a tick; off by default because capture takes the wheel from the terminal's scrollback and plain text selection (T22.4)
