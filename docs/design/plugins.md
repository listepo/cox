...
| Kotlin | none maintained (`LizAinslie/extism-kotlin-pdk`, last push 2023-11-30) | Kotlin/Wasm `wasmWasi`, Beta, WASI preview 1, needs GC and exception handling; `@WasmImport`/`@WasmExport` are experimental | **unproven**: extism turns on GC but enables exceptions only behind its `wasmtime-exceptions` feature (`src/plugin.rs:60-66`); which exception-handling encoding Kotlin emits for WASI is unverified | spike first; if it passes, a thin cox-owned Kotlin PDK over the raw `extism:host/env` imports |
...
3. **Kotlin.** T33.35 spikes Kotlin/Wasm `wasmWasi` first. If it instantiates under the wasmtime 43 extism pins and round-trips `cox_init`, cox owns a thin PDK over the raw `extism:host/env` imports (T33.36) and turns on extism's non-default `wasmtime-exceptions` feature only if the spike needs it.
