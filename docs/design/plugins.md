...
| Kotlin | none maintained (`LizAinslie/extism-kotlin-pdk`, last push 2023-11-30) | Kotlin/Wasm `wasmWasi`, Beta, WASI preview 1, needs GC and exception handling; `@WasmImport`/`@WasmExport` are experimental | **refuted (T33.35, 2026-09-26, R§4.3.5 P40–P43)**: a minimal `cox_init` module (Kotlin 2.4.20) fails to *parse* under extism 1.30.0's default engine config — `exceptions proposal not enabled` — regardless of WASI; Kotlin's `wasmWasi` output unconditionally uses the wasm exception-handling proposal, which extism enables only behind its non-default `wasmtime-exceptions` feature. With that feature on it instantiates and calls `cox_init` cleanly, with WASI off exactly as the real host runs today (A55/T33.43 is not the blocker here) | blocked on `wasmtime-exceptions`, which cox does not enable without the creator's sign-off (it applies to every plugin, not only Kotlin); Kotlin stays out of `--lang`, T33.36 does not proceed |
| Dart | none | `dart2wasm` targets JS environments only: "doesn't support execution in standard Wasm run-times like wasmtime" (dart.dev, Dart 3.13) | **no** | documented exception: the Dart template scaffolds a plugin whose only capability is an `[[mcp]]` stdio server (`dart compile exe`, `dart_mcp` 0.5.2), so `--with` accepts only `tool` and `mcp` for Dart; a spike re-checks dart-lang/sdk#56366 (WASI support, open) |
...
2. **Dart.** Stays the documented exception (§13): its only v1 capability is an `[[mcp]]` stdio server. T33.37 keeps re-checking dart-lang/sdk#56366 so the exception can be lifted later.
3. **Kotlin.** T33.35's spike (2026-09-26, R§4.3.5 P40–P43) refutes it as shipped: a minimal Kotlin/Wasm `wasmWasi` module needs extism's non-default `wasmtime-exceptions` feature just to parse (unrelated to WASI/A55), and cox does not turn that feature on without the creator's sign-off — it would apply to every plugin, not only a future Kotlin one. Kotlin stays out of `--lang`; T33.36 does not proceed unless the creator later approves `wasmtime-exceptions` for the workspace.
